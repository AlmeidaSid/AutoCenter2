...
def update_cliente(request, id):
    ...
    try:
        ...
        cliente.cpf = cpf
        cliente.save()
        return HttpResponse('Atualização feita com sucesso')
    except:
...
def delete_cliente(request, id):
    ...
    if request.method == 'POST':
        ...
        if carros.exists():
            return JsonResponse({'error': 'O cliente possui carros associados. Exclua os carros primeiro.'}, status=400)
        else:
            cliente.delete()
            return JsonResponse({'success': 'Cliente excluído com sucesso.'})
